...
CVTMLE<-function(A,X,Y,scores,candidate.tau,error.bound=0.05,conf.level=.95,nfolds=5,g.control=list(SL.library=c("SL.glm","SL.gam","SL.randomForest")),Q.control=list(SL.library=c("SL.glm","SL.gam","SL.randomForest")),g.trunc=1e-2,select.tau=ifelse(length(candidate.tau)==1,FALSE,TRUE)){
    ...
    for(tau.index in 1:length(candidate.tau)){
        for(v in 1:nfolds){
            ...
            psi.v[v,tau.index]<-sum((1-A.test)*Qhat)/sum(1-A.test)
            IF<-ifelse(A.test,(Z.test-Qhat)*w.test/gamma.test,(Qhat-psi.v[v,tau.index])/(1-gamma.test))
...
# Confidence upper bound of true coverage error or threshold selection based on cross-validated targeted minimum loss-based estimator with known likelihood ratio (via grid search)
# Method to compute a confidence upper bound of true coverage or select a threshold (via grid search) for APAC prediction sets based on cross-validated targeted minimum loss-based estimators with known likelihood ratio
...
CVTMLE_knownLR<-function(A,X,Y,scores,candidate.tau,LR=function(x) 1,error.bound=0.05,conf.level=.95,nfolds=5,Q.control=list(SL.library=c("SL.glm","SL.gam","SL.randomForest")),select.tau=ifelse(length(candidate.tau)==1,FALSE,TRUE)){
    #check inputs
    if(any(!(A %in% c(0,1)))){
        stop("A must take values 0 or 1")
    }
...
#' @title Threshold selection based on cross-validated targeted minimum loss-based estimator via bisection search
#' @name CVTMLE_bisec
#' @export
#' @description Method to select a threshold for APAC prediction sets based on cross-validated targeted minimum loss-based estimators via bisection serach
#' @param A vector of population indicator. 1 for source population, 0 for target population
#' @param X data frame of covariates with each row being one observation
#' @param Y vector of dependent variable/outcome. For data from the target population (`A=0`), set the corresponding entries of `Y` to be `NA`
#' @param scores either a function assigning scores of `Y` given `X` trained using an independent dataset from source population or a vector of this function evaluated at observed `(X,Y)`, taking `NA` for observations from the target population. If it is a function, it must take input `(x,y)`, where `x` is one row of `X` (a data frame with one row) and `y` is a nonmissing value of `Y`, and output a scalar
...
#' Y<-rbinom(n,1,expit(1+X$X))
#' scores<-dbinom(Y,1,expit(.08+1.1*X$X))
#' CVTMLE_bisec(A,X,Y,scores,nfolds=2,
#'              g.control=list(SL.library="SL.glm"),
#'              Q.control=list(SL.library="SL.glm"))
CVTMLE_bisec<-function(A,X,Y,scores,tau.interval=c(0,1),error.bound=0.05,conf.level=.95,nfolds=5,g.control=list(SL.library=c("SL.glm","SL.gam","SL.randomForest")),Q.control=list(SL.library=c("SL.glm","SL.gam","SL.randomForest")),g.trunc=1e-2,tol=1e-2){
    #check inputs
    if(any(!(A %in% c(0,1)))){
        stop("A must take values 0 or 1")
    }
...
# Threshold selection based on cross-validated targeted minimum loss-based estimator via bisection search with known likelihood ratio
# Method to select a threshold for APAC prediction sets based on cross-validated targeted minimum loss-based estimators via bisection search with known likelihood ratio
# A: vector of population indicator. 1 for source population, 0 for target population
# X: data frame of covariates with each row being one observation
# Y: vector of dependent variable/outcome. For data from the target population (`A=0`), set the corresponding entries of `Y` to be `NA`
# scores: either a function assigning scores of `Y` given `X` trained using an independent dataset from source population or a vector of this function evaluated at observed `(X,Y)`, taking `NA` for observations from the target population. If it is a function, it must take input `(x,y)`, where `x` is one row of `X` (a data frame with one row) and `y` is a nonmissing value of `Y`, and output a scalar
...
# scores<-dbinom(Y,1,expit(.08+1.1*X$X))
# LR<-function(x) dnorm(x$X,sd=.5)/dnorm(x$X)
# APACpred:::CVTMLE_knownLR_bisec(A,X,Y,scores,Q.control=list(SL.library="SL.glm"))
CVTMLE_knownLR_bisec<-function(A,X,Y,scores,tau.interval=c(0,1),LR=function(x) 1,error.bound=0.05,conf.level=.95,nfolds=5,Q.control=list(SL.library=c("SL.glm","SL.gam","SL.randomForest")),tol=1e-2){
    #check inputs
    if(any(!(A %in% c(0,1)))){
        stop("A must take values 0 or 1")
    }
...
# Similar to CVTMLE_bisec, but does bisection search over observed scores
# 
# example:
# n<-100
# expit<-function(x) 1/(1+exp(-x))
...
# Y<-rbinom(n,1,expit(1+X$X))
# scores<-dbinom(Y,1,expit(.08+1.1*X$X))
# APACpred:::CVTMLE_bisec2(A,X,Y,scores,nfolds=2,g.control=list(SL.library="SL.glm"),Q.control=list(SL.library="SL.glm"))
CVTMLE_bisec2<-function(A,X,Y,scores,error.bound=0.05,conf.level=.95,nfolds=5,g.control=list(SL.library=c("SL.glm","SL.gam","SL.randomForest")),Q.control=list(SL.library=c("SL.glm","SL.gam","SL.randomForest")),g.trunc=1e-2,tol=1e-2){
    #check inputs
    if(any(!(A %in% c(0,1)))){
        stop("A must take values 0 or 1")
    }
...
# Similar to CVTMLE_knownLR_bisec, but does bisection search over observed scores
# 
# example:
# n<-100
# expit<-function(x) 1/(1+exp(-x))
...
# scores<-dbinom(Y,1,expit(.08+1.1*X$X))
# LR<-function(x) dnorm(x$X,sd=.5)/dnorm(x$X)
# APACpred:::CVTMLE_knownLR_bisec2(A,X,Y,scores,Q.control=list(SL.library="SL.glm"))
CVTMLE_knownLR_bisec2<-function(A,X,Y,scores,LR=function(x) 1,error.bound=0.05,conf.level=.95,nfolds=5,Q.control=list(SL.library=c("SL.glm","SL.gam","SL.randomForest")),tol=1e-2){
    #check inputs
    if(any(!(A %in% c(0,1)))){
        stop("A must take values 0 or 1")
    }
